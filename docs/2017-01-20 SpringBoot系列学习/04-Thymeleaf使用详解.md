...
| th:if       | 判断条件                                     | `<a th:if="${userId == collect.userId}" ><br/>th:unless	和th:if判断相反	<a th:href="@{/login}" th:unless=${session.user != null}>Login</a>` |
...
| th:attr     | 设置标签属性，多个属性可以用逗号分隔         | 如`<th:attr="src=@{/image/aa.jpg},title=#{logo}">`，此标签不太优雅，一般用的比较少。 |
